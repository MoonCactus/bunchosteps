...
flash:	all
	$(AVRDUDE) -U flash:w:manysteps.hex:i
...
load: all
	bootloadHID manysteps.hex

clean:
	rm -f manysteps.hex $(BUILDDIR)/*.o $(BUILDDIR)/*.d $(BUILDDIR)/*.elf

# file targets:
$(BUILDDIR)/main.elf: $(OBJECTS)
	$(COMPILE) -o $(BUILDDIR)/main.elf $(OBJECTS) -lm -Wl,--gc-sections

manysteps.hex: $(BUILDDIR)/main.elf
	rm -f manysteps.hex
	avr-objcopy -j .text -j .data -O ihex $(BUILDDIR)/main.elf manysteps.hex
